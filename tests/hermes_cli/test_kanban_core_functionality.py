...
def test_gateway_dispatcher_throttles_transient_disk_io_errors(
    monkeypatch, tmp_path, caplog
):
    """Repeated sqlite disk I/O tick failures warn once until the DB file changes."""
    import asyncio
    import logging
    import sqlite3
    ...
    messages = [record.getMessage() for record in caplog.records]
    assert sum("transient SQLite disk I/O error" in msg for msg in messages) == 1
    assert not any("tick failed on board" in msg for msg in messages)
    assert not any(record.exc_info for record in caplog.records)

...
describe.skipIf(onWindows)('execFileNoThrow with daemon-style children', () => {
  ...
  it("settles immediately on 'exit' when resolveOnExit is true, regardless of daemon stdio", async () => {
    ...
    // of waiting for the orphaned sleeper to release stdio. Keep the
    // bound below the 2s timeout and far below the sleeper's 3s runtime,
    // while leaving room for busy developer machines.
